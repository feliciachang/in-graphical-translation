...
function createWord(text, width, len, wordHeight, wordWidth) {
  ...
  console.log(width, canvasWidth, len);
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    console.log(y, "y", canvasHeight, lineHeight);
    counter++;
    switch (text[i]) {
      case "A":
      ...
      case "P":
        left(x, width, height, canvasTop + y, wordHeight + y);
        topH(x, width, height, canvasTop + y, letterWidth);
        right(x, width, height, canvasTop + y, wordHeight / 2 + y, letterWidth);
        middleH(
          x,
          width,
          height,
